...
const calculatePremium = ({ city, zone, platform, avgWeeklyIncome, historicalClaims = 0 }) => {
  ...
  // Each prior claim adds 3% to premium, max 30% surcharge
  const historyFactor = 1 + Math.min(historicalClaims * 0.03, 0.30);

  // ── Income-proportional coverage floor ────────────────────────────────────
  // Premium should be ~4-6% of avg weekly income (actuarially fair)
  ...
  // Blend model output with income-based anchor (60/40 split)
  const blendedPremium = (rawPremium * 0.6) + (incomeBasedPremium * 0.4);
  ...
  return {
    ...
    riskScore: +((weatherFactor + pollutionFactor + zoneFactor + platformFactor + historyFactor - 5) / 5).toFixed(3),
    recommendation: weeklyPremium <= 150 ? 'Low Risk — Great Coverage Value' :
                    weeklyPremium <= 250 ? 'Moderate Risk — Recommended' :
                    'High Risk Zone — Essential Coverage',
  };
};
