...
const getDashboard = (req, res) => {
  ...
  // Fraud Profile Breakdown
  const fraudProfile = db.prepare(`
    SELECT 
      SUM(CASE WHEN fraud_score >= 0.7 THEN 1 ELSE 0 END) as high,
      SUM(CASE WHEN fraud_score >= 0.4 AND fraud_score < 0.7 THEN 1 ELSE 0 END) as medium,
      SUM(CASE WHEN fraud_score < 0.4 THEN 1 ELSE 0 END) as low
    FROM claims
  `).get();
  ...
  res.json({
    summary: { totalUsers, activePolicies, totalPremium, totalPayouts, pendingClaims, fraudAlerts, lossRatio, fraudProfile },
    claimsByType,
    claimsTrend,
    riskByCities,
    recentFraud,
